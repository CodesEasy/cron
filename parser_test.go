...
func TestAll(t *testing.T) {
	allBits := []struct {
		...
		{dow, 0xff},                  // 0-7: 8 ones (7 is also Sunday)
	}
...
func TestParseScheduleErrors(t *testing.T) {
	var tests = []struct{ expr, err string }{
		...
		// TZ prefix panics (Phase 1.1)
		{"TZ=0", "missing fields after timezone"},
		{"TZ=", "missing fields after timezone"},
		{"CRON_TZ=Asia/Tokyo", "missing fields after timezone"},
...
func TestStandardSpecSchedule(t *testing.T) {
	entries := []struct {
		...
		{
			expr:     "5 * * * *",
			expected: &SpecSchedule{1 << seconds.min, 1 << 5, all(hours), all(dom), all(months), normalizeDOW(all(dow)), time.Local},
		},
		{
			expr:     "@every 5m",
...
func every5min(loc *time.Location) *SpecSchedule {
	return &SpecSchedule{1 << 0, 1 << 5, all(hours), all(dom), all(months), normalizeDOW(all(dow)), loc}
}

func every5min5s(loc *time.Location) *SpecSchedule {
	return &SpecSchedule{1 << 5, 1 << 5, all(hours), all(dom), all(months), normalizeDOW(all(dow)), loc}
}

func midnight(loc *time.Location) *SpecSchedule {
	return &SpecSchedule{1, 1, 1, all(dom), all(months), normalizeDOW(all(dow)), loc}
}
...
func TestNewParserPanicsWithNoFields(t *testing.T) {
	defer func() {
		if r := recover(); r == nil {
			t.Error("expected NewParser(0) to panic")
		}
	}()
	NewParser(0)
}

func TestNewParserDescriptorOnly(t *testing.T) {
	// Descriptor-only parser should not panic
	p := NewParser(Descriptor)
	_, err := p.Parse("@hourly")
	if err != nil {
		t.Errorf("expected descriptor-only parser to work, got %v", err)
	}
}

func TestSunday7(t *testing.T) {
	// Sunday=7 should equal Sunday=0
	sun0, err := standardParser.Parse("* * * * 0")
	if err != nil {
		t.Fatal(err)
	}
	sun7, err := standardParser.Parse("* * * * 7")
	if err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(sun0, sun7) {
		t.Errorf("Sunday=0 and Sunday=7 should produce identical schedules\n  0: %+v\n  7: %+v", sun0, sun7)
	}

	// 0-7 should equal * (all days)
	allDays, err := standardParser.Parse("* * * * *")
	if err != nil {
		t.Fatal(err)
	}
	range07, err := standardParser.Parse("* * * * 0-7")
	if err != nil {
		t.Fatal(err)
	}
	// Both have starBit cleared for the range version, so compare Dow bits directly
	allSpec := allDays.(*SpecSchedule)
	rangeSpec := range07.(*SpecSchedule)
	allDowBits := allSpec.Dow &^ starBit
	rangeDowBits := rangeSpec.Dow &^ starBit
	if allDowBits != rangeDowBits {
		t.Errorf("0-7 DOW bits should equal * DOW bits\n  *: %b\n  0-7: %b", allDowBits, rangeDowBits)
	}

	// 6-7 should match Saturday and Sunday (bits 0 and 6)
	sat_sun, err := standardParser.Parse("* * * * 6-7")
	if err != nil {
		t.Fatal(err)
	}
	satSunSpec := sat_sun.(*SpecSchedule)
	expected := uint64(1<<0 | 1<<6) // Sunday=0 and Saturday=6
	if satSunSpec.Dow != expected {
		t.Errorf("6-7 should have bits 0 and 6 set, got %b", satSunSpec.Dow)
	}
}

func TestValidSteps(t *testing.T) {
	// */59 in minutes should be valid (step < range size of 60)
	_, err := standardParser.Parse("*/59 * * * *")
	if err != nil {
		t.Errorf("*/59 should be valid: %v", err)
	}
	// */30 should work
	_, err = standardParser.Parse("*/30 * * * *")
	if err != nil {
