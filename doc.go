...
Day-of-week 7 is Sunday (same as 0), per POSIX crontab convention.
...
For example:
	...
	# Runs at 6am in Asia/Tokyo, overriding the instance-level location
	c := cron.New(cron.WithLocation(nyc))
...
to achieve the following effects:
  ...
  - Recover any panics from jobs (activated by default, using the configured logger)
  - Delay a job's execution if the previous run hasn't completed yet
  - Skip a job's execution if the previous run hasn't completed yet
  - Log each job's invocations

Install wrappers for all jobs added to a cron using the `cron.WithChain` option.
Note that this replaces the default chain entirely, so panic recovery will not
be active unless you include Recover explicitly:
...
To disable all wrappers (including default panic recovery), pass an empty chain:
