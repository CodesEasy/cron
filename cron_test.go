...
func TestStopAndWait_RepeatedStop(t *testing.T) {
	cron := newWithSeconds()
	cron.Start()
	_ = cron.Stop()
...
func TestStopAndWait_SlowJob(t *testing.T) {
	cron := newWithSeconds()
	cron.AddFunc("* * * * * *", func() {})
	cron.Start()
...
func TestStopAndWait_RepeatedStopWithCompletion(t *testing.T) {
	cron := newWithSeconds()
	cron.AddFunc("* * * * * *", func() {})
	cron.AddFunc("* * * * * *", func() { time.Sleep(2 * time.Second) })
	cron.Start()
...
func TestDefaultRecoverJob(t *testing.T) {
	// New() without WithChain — default Recover should be active
	c := New()
	wrapped := c.chain.Then(FuncJob(func() { panic("test panic") }))
	// Should not panic — Recover catches it
	wrapped.Run()
}

func TestDefaultRecoverUsesCustomLogger(t *testing.T) {
	var buf syncWriter
	c := New(WithLogger(newBufLogger(&buf)))
	wrapped := c.chain.Then(FuncJob(func() { panic("custom logger test") }))
	wrapped.Run()
	if !strings.Contains(buf.String(), "custom logger test") {
		t.Error("expected panic logged to custom logger")
	}
}

func TestMultipleZeroTimeEntries(t *testing.T) {
	cron := newWithSeconds()
	var calls int64
	cron.AddFunc("* * * * * *", func() { atomic.AddInt64(&calls, 1) })
	cron.Schedule(new(ZeroSchedule), FuncJob(func() { t.Error("zero schedule 1 should not run") }))
	cron.Schedule(new(ZeroSchedule), FuncJob(func() { t.Error("zero schedule 2 should not run") }))
	cron.Start()
	defer cron.Stop()
	<-time.After(OneSecond)
...
func TestWithChainOptOut(t *testing.T) {
	c := New(WithChain()) // explicit empty chain
	wrapped := c.chain.Then(FuncJob(func() { panic("should propagate") }))
	defer func() {
		if r := recover(); r == nil {
			t.Error("expected panic to propagate with empty chain")
		}
	}()
	wrapped.Run()
}
