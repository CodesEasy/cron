...
func TestSkipIfStillRunning_PanicRecovery(t *testing.T) {
	var count int32

	// Wrap: Recover (outermost) → SkipIfStillRunning → panicking job
	wrappedJob := NewChain(Recover(DiscardLogger), SkipIfStillRunning(DiscardLogger)).
		Then(FuncJob(func() {
			atomic.AddInt32(&count, 1)
			panic("test panic")
		}))

	// Run 3 times sequentially — each should execute (not skip)
	// because the token is returned via defer even after panic
	wrappedJob.Run()
	wrappedJob.Run()
	wrappedJob.Run()

	if c := atomic.LoadInt32(&count); c != 3 {
		t.Errorf("expected 3 runs (token recovered each time), got %d", c)
	}
}
